...
def validate_stock_request(ticker: str, start_date: str, end_date: str) -> tuple[str, date, date]:
    ...
    if normalized_ticker not in SUPPORTED_TICKERS:
        raise StockDataError('Nieobslugiwany ticker.')

    parsed_start = parse_date(start_date, 'start_date')
    parsed_end = parse_date(end_date, 'end_date')

    if parsed_start >= parsed_end:
        raise StockDataError('Data poczatkowa musi byc wczesniejsza niz koncowa.')
...
def load_stock_prices(ticker: str, start_date: str, end_date: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise StockDataError('Brak biblioteki yfinance. Zainstaluj zaleznosci z requirements.txt.') from exc
    ...
    if missing_columns:
        raise StockDataError('Pobrane dane nie zawieraja wymaganych kolumn.')
    ...
    if len(prices) < 2:
        raise StockDataError('Zakres musi zawierac co najmniej dwa dni notowan.')

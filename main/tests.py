...
class SimulatorEngineTests(SimpleTestCase):
    ...
    def test_serialize_state_exposes_only_current_snapshot(self):
        ...
        self.assertEqual(serialized['prediction_evaluation_history'], [])
...
class SimulationApiTests(SimpleTestCase):
    ...
    def test_start_simulation_returns_first_day_state_and_model_params(self, load_stock_prices_mock, build_prediction_artifacts_mock):
        ...
        self.assertIsNone(payload['model_metrics']['metrics'])
        self.assertIsNone(payload['model_metrics']['train_rows'])
        self.assertIsNone(payload['model_metrics']['test_rows'])
    ...
    def test_decision_updates_session_and_advances_by_one_day(self, _load_stock_prices_with_history, _build_prediction_artifacts):
        ...
        self.assertEqual(len(payload['prediction_evaluation_history']), 1)
        self.assertEqual(payload['prediction_evaluation_history'][0]['target_date'], '2024-01-03')
        self.assertEqual(payload['prediction_evaluation_history'][0]['actual_close'], '110.00')
        self.assertEqual(payload['prediction_evaluation_history'][0]['error'], '-1.00')
        self.assertTrue(payload['prediction_evaluation_history'][0]['direction_match'])
        self.assertEqual(payload['model_metrics']['train_rows'], 30)
        self.assertEqual(payload['model_metrics']['test_rows'], 10)
        self.assertEqual(payload['model_metrics']['metrics']['classification']['precision'], 0.75)
        self.assertEqual(payload['data_stats']['visible_days'], 2)
